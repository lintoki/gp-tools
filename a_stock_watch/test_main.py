...
class AStockWatchTest(unittest.TestCase):
    ...
    def test_realtime_quotes_falls_back_to_akshare_when_ulist_and_tencent_fail(self):
        item = WatchItem("沪电股份", "002463.SZ", "SZ", 142.0, 144.0, 100, "AI_PCB核心", 1, True)
        ...
        with patch("main.fetch_eastmoney_ulist_quotes", side_effect=RuntimeError("ulist failed")):
            with patch("main.fetch_tencent_realtime_quotes", side_effect=RuntimeError("tencent failed")):
                with patch.dict(sys.modules, {"akshare": fake_akshare}):
                    quotes = fetch_realtime_quotes([item])

        self.assertEqual(quotes["002463"].latest_price, 143.2)

    def test_tencent_realtime_quotes_parse_selected_symbols(self):
        item = WatchItem("沪电股份", "002463.SZ", "SZ", 142.0, 144.0, 100, "AI_PCB核心", 1, True)
        ...
        class FakeResponse:
            ...
            def read(self):
                return (
                    'v_sz002463="51~沪电股份~002463~145.26~147.90~149.79~908645~404136~504509~'
                    '145.26~5947~145.25~435~145.24~74~145.23~165~145.22~51~145.27~55~145.28~'
                    '62~145.29~22~145.30~323~145.31~37~~20260622161418~-2.64~-1.78~151.90~'
                    '141.45~145.26/908645/13288650199~908645~1328865~4.73~64.98";'
                ).encode("gbk")

        with patch("urllib.request.urlopen", return_value=FakeResponse()) as urlopen:
            quotes = fetch_tencent_realtime_quotes([item])

        self.assertEqual(quotes["002463"].name, "沪电股份")
        self.assertEqual(quotes["002463"].latest_price, 145.26)
        self.assertEqual(quotes["002463"].change_pct, -1.78)
        self.assertEqual(quotes["002463"].amount, 13288650199.0)
        self.assertIn("sz002463", urlopen.call_args.args[0].full_url)

    def test_realtime_quotes_falls_back_to_tencent_when_eastmoney_fails(self):
        item = WatchItem("沪电股份", "002463.SZ", "SZ", 142.0, 144.0, 100, "AI_PCB核心", 1, True)
        tencent_quotes = {
            "002463": types.SimpleNamespace(
                code="002463",
                name="沪电股份",
                latest_price=145.26,
                change_pct=-1.78,
                amount=13288650199.0,
            )
        }

        with patch("main.fetch_eastmoney_ulist_quotes", side_effect=RuntimeError("ulist failed")):
            with patch("main.fetch_tencent_realtime_quotes", return_value=tencent_quotes):
                quotes = fetch_realtime_quotes([item])
