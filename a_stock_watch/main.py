...
def _market_prefix_symbol(item: WatchItem) -> str:
    market = item.market.upper() or item.code.split(".", 1)[-1].upper()
    if market == "SH":
        prefix = "sh"
    elif market == "SZ":
...
def fetch_tencent_realtime_quotes(items: Iterable[WatchItem], timeout: int = 10) -> Dict[str, Quote]:
    unique_items = {item.symbol: item for item in items}
    if not unique_items:
        return {}

    symbols = ",".join(_market_prefix_symbol(item) for item in unique_items.values())
    request = urllib.request.Request(
        TENCENT_REALTIME_URL.format(symbols=symbols),
        headers={
            "User-Agent": "Mozilla/5.0",
    ...
    try:
        ...
        raise RuntimeError(f"腾讯行情接口失败: {exc}") from exc
    ...
    for raw_line in text.splitlines():
        ...
        if not line.startswith("v_"):
            continue
        variable, _, raw_value = line.partition("=")
        symbol = variable[-6:].zfill(6)
        if symbol not in unique_items:
            continue

        fields = raw_value.strip().strip('"').split("~")
        if len(fields) < 36 or not fields[1]:
            continue

        amount = None
        amount_parts = fields[35].split("/")
        if len(amount_parts) >= 3:
            amount = _to_float(amount_parts[2])
    ...
    if not quotes:
        raise RuntimeError(f"腾讯行情接口未返回监控股票: {symbols}")
    return quotes
...
def fetch_realtime_quotes(items: Iterable[WatchItem]) -> Dict[str, Quote]:
    ...
    try:
        ...
        errors.append(f"东方财富小范围接口: {exc}")

    try:
        return fetch_tencent_realtime_quotes(items)
    except Exception as exc:
        errors.append(f"腾讯行情接口: {exc}")

    try:
        return fetch_akshare_realtime_quotes(items)
    except Exception as exc:
        errors.append(f"AKShare 全市场接口: {exc}")

    raise RuntimeError(f"行情获取失败；{'; '.join(errors)}")
